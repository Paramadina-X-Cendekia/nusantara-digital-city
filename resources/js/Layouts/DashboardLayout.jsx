...
export default function DashboardLayout({ children }) {
    ...
    const menuItems = isAdmin ? [
        { label: 'Admin Dashboard', icon: 'grid_view', href: '/dashboard' },
        { label: 'Moderasi Data', icon: 'verified_user', href: '/dashboard' },
        { label: 'Kembali ke Situs', icon: 'arrow_back', href: '/' },
    ] : [
        { label: 'Dashboard Saya', icon: 'dashboard', href: '/dashboard' },
        { label: 'Kontribusi Baru', icon: 'add_box', href: '/kontribusi' },
        { label: 'Kembali ke Situs', icon: 'arrow_back', href: '/' },
    ];
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-display flex antialiased">
            ...
            <aside className={`bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 flex flex-col ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
                <div className="p-6 flex items-center gap-3 overflow-hidden">
                    ...
                    {isSidebarOpen && <span className="font-black italic text-lg whitespace-nowrap">{isAdmin ? 'Admin' : 'User'} <span className="text-primary">Panel</span></span>}
                </div>
                ...
                <div className="p-4 mt-auto">
                    ...
                    >
                        ...
                        {isSidebarOpen && <span className="font-bold text-sm whitespace-nowrap">Keluar</span>}
                    </button>
                </div>
            </aside>
                ...
                <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="text-right hidden sm:block">
                            ...
                            <p className="text-[10px] text-primary font-bold uppercase tracking-widest mt-1">{isAdmin ? 'Administrator' : 'Kontributor'}</p>
                        </div>
