...
import NusantaraDigitalBackground from "@/components/NusantaraDigitalBackground";
    ...
    useEffect(() => {
        ...
        function resize() {
            ...
            for (let r = 0; r < rows; r++)
                for (let c = 0; c < cols; c++)
                    cellsRef.current.push({ r, c, lit: 0 });
        }
        ...
        function draw() {
            ...
            cellsRef.current.forEach((cell) => {
                const cx = cell.c * CELL;
                const cy = cell.r * CELL;
                const dx = lx - (cx + CELL / 2);
                const dy = ly - (cy + CELL / 2);
                const dist = Math.sqrt(dx * dx + dy * dy);
                const target = Math.max(0, 1 - dist / 200);
                cell.lit = lerp(cell.lit, target, 0.08);

                // Grid lines
                ctx.strokeStyle = `rgba(59,130,246,${0.06 + cell.lit * 0.18})`;
                ctx.lineWidth = 0.5;
                ctx.strokeRect(cx, cy, CELL, CELL);

                // Cell fill on hover
                if (cell.lit > 0.03) {
                    ctx.fillStyle = `rgba(59,130,246,${cell.lit * 0.07})`;
...
function HeroScrambleWord({ words }) {
    ...
    useEffect(() => {
        ...
        async function init() {
            ...
            // Render karakter sebagai span individual dengan wrapper overflow hidden
            function renderWord(word) {
                container.innerHTML = word
                    .split("")
                    .map(
                        (ch) =>
                            `<span class="char-wrap" style="display:inline-block;overflow:visible;line-height:1.2;vertical-align:bottom;">` +
                            `<span class="char-inner" style="display:inline-block;">${ch === " " ? "&nbsp;" : ch}</span>` +
                            `</span>`,
                    )
                    .join("");
                return Array.from(container.querySelectorAll(".char-inner"));
            }

            // Animasi masuk — bounce dari bawah
            async function animateIn(chars) {
                // Reset posisi dulu
                chars.forEach((ch) => {
                    ch.style.transform = "translateY(100%) rotate(-12deg)";
                    ch.style.opacity = "0";
                });
                ...
                await animate(chars, {
                    translateY: [
                        { to: "100%", duration: 0 },
                        { to: "-6%", ease: "outExpo", duration: 550 },
                        {
                            to: "0%",
                            ease: "outBounce",
                            duration: 500,
                    ...
                    rotate: [
                        { to: -12, duration: 0 },
                        { to: 2, ease: "outExpo", duration: 550 },
                        { to: 0, ease: "outBounce", duration: 500, delay: 60 },
                    ],
                    opacity: [
                        { to: 0, duration: 0 },
                        { to: 1, ease: "linear", duration: 150 },
                    ],
                    delay: stagger(40),
                }).finished;
            }

            // Animasi keluar — slide up ke atas
            async function animateOut(chars) {
                await animate(chars, {
                    translateY: { to: "-105%", ease: "inExpo", duration: 380 },
                    opacity: { to: 0, ease: "linear", duration: 250 },
                    delay: stagger(25, { from: "last" }),
                }).finished;
            }

            // Tampilkan kata pertama
            const firstChars = renderWord(words[0]);
            await animateIn(firstChars);

            // Loop ganti kata
            intervalRef.current = setInterval(async () => {
                if (destroyed || isAnimating.current) return;
                isAnimating.current = true;

                const container = containerRef.current;
                if (!container) return;

                // Ambil chars yang sedang tampil
                const currentChars = Array.from(
                    container.querySelectorAll(".char-inner"),
                );
                ...
                const newChars = renderWord(words[indexRef.current]);

                // Masuk
                await animateIn(newChars);
...
export default function Home({ leaderboard = [] }) {
    ...
    const PILARS = [
        {
            icon: "history_edu",
            title: t("home.pillar1_title"),
            desc: t("home.pillar1_desc"),
        },
        {
            icon: "map",
            title: t("home.pillar2_title"),
            desc: t("home.pillar2_desc"),
        },
        {
            icon: "storefront",
            title: t("home.pillar3_title"),
            desc: t("home.pillar3_desc"),
        },
        {
            icon: "hub",
            title: t("home.pillar4_title"),
            desc: t("home.pillar4_desc"),
        },
    ];

    // Handler mouse untuk grid hero
        ...
        if (sectionRef.current && triggerRef.current) {
            ...
            if (heroRef.current) {
                if (heroBgRef.current) {
                    gsap.set(heroBgRef.current, { force3D: true });
                    heroBgScroll = gsap.to(heroBgRef.current, {
                        yPercent: 12, // dikurangi dari 15
                        ease: "none",
                        scrollTrigger: {
                            trigger: heroRef.current,
                            start: "top top",
                            end: "bottom top",
                            scrub: 1.2, // naikkan sedikit biar lebih smooth
                ...
                if (heroContentRef.current) {
                    gsap.set(heroContentRef.current, { force3D: true });
                    heroContentScroll = gsap.to(heroContentRef.current, {
                        yPercent: -12,
                        opacity: 0.85, // tidak sampai 0 supaya tidak terlalu "hilang"
                        ease: "none",
                        scrollTrigger: {
                            trigger: heroRef.current,
                            start: "top top",
    ...
    return (
        <div className="relative flex min-h-screen flex-col overflow-x-hidden font-display text-slate-900 dark:text-slate-300 transition-colors duration-300 antialiased">
            ...
            <main className="flex-grow">
                ...
                {/* Hero Section — Grid Background + Anime.js Scramble */}
                <section
                    ref={heroRef}
                ...
                >
                    {/* Grid Background */}
                    {/* Nusantara Digital City Background */}
                    <div
                        ref={heroBgRef}
                        className="absolute inset-0 pointer-events-none overflow-hidden"
                        style={{ willChange: "transform" }}
                    >
                        {/* Gradient dasar — biru malam ke amber Nusantara */}
                        <div className="absolute inset-0 bg-gradient-to-b from-amber-50/40 via-white to-blue-50/60 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950" />

                        {/* Glow radial atas */}
                        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_-10%,rgba(234,88,12,0.10),transparent_60%)] dark:bg-[radial-gradient(ellipse_80%_60%_at_50%_-10%,rgba(59,130,246,0.20),transparent_60%)]" />

                        {/* Pattern Nusantara interaktif */}
                        <NusantaraDigitalBackground
                            mouseX={mousePos.x}
                            mouseY={mousePos.y}
                        />

                        {/* Vignette — FIX: pakai warna konkret, bukan var tak ada */}
                        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_40%,white_100%)] dark:bg-[radial-gradient(ellipse_at_center,transparent_40%,rgb(2_6_23)_100%)]" />
                    </div>
                    ...
                    >
                        <div className="w-full px-4 sm:px-10 lg:px-20 py-24 md:py-32 flex flex-col items-center">
                            <div className="max-w-4xl flex flex-col items-center space-y-6">
                                ...
                                {/* Judul dengan Scramble Text */}
                                <h1
                                    data-hero-stagger
                                    className="text-5xl sm:text-6xl lg:text-7xl font-black text-slate-900 dark:text-white leading-[1.2] tracking-tight text-center max-w-4xl mx-auto"
                                >
                                ...
                                <p
                                    data-hero-stagger
                                    className="text-base sm:text-lg lg:text-xl text-slate-600 dark:text-slate-300 font-medium leading-relaxed max-w-2xl text-center mx-auto"
                                >
                                ...
                                >
                                    ...
                                    <motion.button
                                        ...
                                        className="rounded-xl h-12 px-8 bg-slate-100/70 dark:bg-white/10 backdrop-blur-md text-slate-800 dark:text-white border border-slate-200 dark:border-white/20 text-sm font-bold hover:bg-slate-200/70 dark:hover:bg-white/20 transition-colors"
                                    >
                                        {t("home.cta_learn")}
                                    </motion.button>
                                </div>
                ...
                >
                    <div
                        ref={sectionRef}
                        className="flex h-[90vh] items-center px-10 md:px-24 gap-16 md:gap-32 w-max"
                    >
                        {/* Title Card */}
                ...
                <section className="py-24 px-4 bg-white/30 dark:bg-slate-900/20  overflow-hidden">
                    <div className="container mx-auto max-w-6xl">
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-20 gap-10">
                            ...
                            <p className="text-lg text-slate-500 dark:text-slate-400 max-w-sm font-medium text-left">
                                {t("home.how_desc")}
                            </p>
                        </div>
                ...
                <section className="py-24 px-4 relative overflow-hidden bg-white/40 dark:bg-slate-950/40 ">
                    <div className="container mx-auto max-w-6xl relative z-10">
                        ...
                        >
                            {[
                                {
                                    title: t("home.why1_title"),
                                    desc: t("home.why1_desc"),
                                    icon: "bolt",
                                    color: "bg-blue-500",
                                },
                                {
                                    title: t("home.why2_title"),
                                    desc: t("home.why2_desc"),
                                    icon: "auto_stories",
                                    color: "bg-emerald-500",
                                },
                                {
                                    title: t("home.why3_title"),
                                    desc: t("home.why3_desc"),
                                    icon: "public",
                                    color: "bg-amber-500",
                                },
                                {
                                    title: t("home.why4_title"),
                                    desc: t("home.why4_desc"),
                                    icon: "groups",
                                    color: "bg-purple-500",
                                },
                            ].map((item, idx) => (
                ...
                <section className="py-24 px-4 bg-white/40 dark:bg-slate-900/50  text-slate-900 dark:text-white overflow-hidden relative transition-colors duration-300">
                    ...
                    <div className="container mx-auto max-w-7xl relative z-10">
                        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between mb-20 gap-8">
                            ...
                            <p className="text-xl text-slate-500 dark:text-slate-400 max-w-md font-medium text-left lg:text-right border-l-2 lg:border-l-0 lg:border-r-2 border-primary pl-6 lg:pl-0 lg:pr-6 py-2">
                                {t("home.feat_desc")}
                            </p>
                        </div>
                                                ...
                                                <div className="flex flex-col h-full items-center justify-center text-center space-y-8 pb-12">
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="inline-flex gap-2 p-1 bg-slate-200/50 dark:bg-white/5 rounded-full border border-slate-300 dark:border-white/10">
                                                            <button className="px-4 py-1.5 bg-primary text-white rounded-full text-[10px] font-black uppercase">
                                                                Model 3D
                                                            </button>
                                                            <button className="px-4 py-1.5 text-slate-600 dark:text-slate-400 text-[10px] font-black uppercase">
                                                                AR Mode
                                                            </button>
                                                        </div>
                                            ...
                                            {activeFeature === 1 && (
                                                <div className="space-y-6 h-full pb-32 overflow-y-auto custom-scrollbar pr-2">
                                                    ...
                                                    <div className="relative">
                                                        <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 material-symbols-outlined text-sm">
                                                            search
                                                        </div>
                                                    ...
                                                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                                                        {[
                                                            ...
                                                            <div
                                                                key={i}
                                                                className={`px-3 py-1 rounded-full text-[9px] font-black uppercase border whitespace-nowrap cursor-default ${i === 0 ? "bg-primary border-primary text-white" : "bg-slate-200/50 dark:bg-white/5 border-slate-300 dark:border-white/10 text-slate-600 dark:text-slate-400"}`}
                                                            >
                                                                {cat}
                                                            </div>
                                                        ))}
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4 pb-8">
                                                        {[
                                                            ...
                                                            >
                                                                <div
                                                                    className={`aspect-[4/3] bg-gradient-to-br ${item.color} to-slate-200/50 dark:to-slate-900/50 flex items-center justify-center relative`}
                                                                >
                                                                    <span className="material-symbols-outlined text-3xl text-slate-400 dark:text-white/20 group-hover:scale-110 transition-transform">
                                                                        {
                                                                            item.icon
                                                                        }
                                                                    </span>
                                                                    <div className="absolute top-2 right-2 px-1.5 py-0.5 rounded-md bg-white/50 dark:bg-white/10 backdrop-blur-md text-[7px] font-black uppercase tracking-widest text-slate-600 dark:text-white/50">
                                                                        {
                                                                            item.cat
                                                                        }
                                                                    </div>
                                                                ...
                                                                <div className="p-3 space-y-1">
                                                                    ...
                                                                    <p className="text-[9px] font-black uppercase truncate text-slate-700 dark:text-slate-300 group-hover:text-primary transition-colors">
                                                                        {
                                                                            item.title
                                                                        }
                                                                    </p>
                                                ...
                                                <div className="relative h-full pb-12 rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/10">
                                                    <div className="absolute inset-0 bg-slate-100 dark:bg-slate-900 grid grid-cols-12 grid-rows-12 opacity-30">
                                                        {Array.from({
                                                            ...
                                                            <div
                                                                key={i}
                                                                className="border-[0.5px] border-slate-300 dark:border-white/5"
                                                            ></div>
                                                        ))}
                                                    </div>
                                                        ...
                                                        <div className="flex -space-x-3">
                                                            {[1, 2, 3].map(
                                                                (i) => (
                                                                    <div
                                                                        key={i}
                                                                        className="size-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-300 dark:bg-slate-700"
                                                                    ></div>
                                                                ),
                                                            )}
                                                        </div>
                ...
                <section className="py-24 px-4 bg-white/40 dark:bg-slate-950/40  overflow-hidden">
                    <div className="container mx-auto max-w-6xl">
                        ...
                        <div
                            ref={timelineRef}
                            className="badge-timeline-container relative max-w-5xl mx-auto mt-16 pt-10"
                        >
                            {/* Central Line */}
                            ...
                            <div className="space-y-16">
                                {[
                                    {
                                        title: t("home.badge_1_title"),
                                        desc: t("home.badge_1_desc"),
                                        color: "from-amber-400 to-amber-600",
                                        icon: "explore",
                                        level: "Level 1 (0-3 Kontribusi)",
                                    },
                                    {
                                        title: t("home.badge_2_title"),
                                        desc: t("home.badge_2_desc"),
                                        color: "from-blue-400 to-blue-600",
                                        icon: "history_edu",
                                        level: "Level 2 (4-10 Kontribusi)",
                                    },
                                    {
                                        title: t("home.badge_3_title"),
                                        desc: t("home.badge_3_desc"),
                                        color: "from-emerald-400 to-emerald-600",
                                        icon: "shield",
                                        level: "Level 3 (11-20 Kontribusi)",
                                    },
                                    {
                                        title: t("home.badge_4_title"),
                                        desc: t("home.badge_4_desc"),
                                        color: "from-purple-400 to-purple-600",
                                        icon: "military_tech",
                                        level: "Level 4 (21+ Kontribusi)",
                                    },
                                ].map((badge, idx) => (
                                    <div
                                        key={idx}
                                        className={`timeline-item relative flex flex-col md:flex-row items-center gap-6 md:gap-10 lg:gap-20 ${idx % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}
                                    >
                                        {/* Icon Side */}
                                        <div
                                            className={`badge-icon-anim flex-1 w-full flex justify-center ${idx % 2 === 0 ? "md:justify-end" : "md:justify-start"}`}
                                        >
                                            <div className="relative p-6 rounded-[2.5rem] bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center hover:shadow-2xl hover:shadow-primary/5 transition-all duration-300 max-w-[240px] md:max-w-[280px] w-full">
                                                <div
                                                    className={`size-24 md:size-32 rounded-full bg-gradient-to-br ${badge.color} flex items-center justify-center p-1 shadow-xl hover:scale-110 transition-transform duration-500`}
                                                >
                                        ...
                                        <div
                                            className={`badge-text-anim flex-1 w-full flex flex-col justify-center items-center text-center ${idx % 2 === 0 ? "md:items-start md:text-left" : "md:items-end md:text-right"}`}
                                        >
                                            <div
                                                className={`space-y-3 max-w-sm ${idx % 2 === 0 ? "" : "md:ml-auto"} flex flex-col items-center ${idx % 2 === 0 ? "md:items-start" : "md:items-end"}`}
                                            >
