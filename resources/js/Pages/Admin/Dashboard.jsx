...
export default function AdminDashboard({ auth, allContributions = [], pendingCount = 0 }) {
    ...
    const handleDelete = (id) => {
        router.delete(`/admin/contributions/${id}`, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const tabContributions = allContributions.filter(c => {
    ...
    const filteredContributions = tabContributions.filter(c => {
        // Status filter
        if (statusFilter !== 'all' && c.status !== statusFilter) return false;

        // Search query filter
        const entityName = (c.data.shopName || c.data.artName || c.data.cityName || c.data.dishName || 'Unnamed').toLowerCase();
        const contributorName = (c.user?.name || '').toLowerCase();
        const query = searchQuery.toLowerCase();

        return entityName.includes(query) || contributorName.includes(query);
    });
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-black tracking-tight">{t('dashboard.moderation_title')}</h1>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div className="relative flex-1 min-w-[240px]">
                            ...
                            <input
                                type="text"
                                placeholder="Cari entitas / kontributor..."
                                value={searchQuery}
                                onChange={(e) => {
                        ...
                        <select
                            ...
                            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm font-bold focus:ring-2 focus:ring-primary outline-none transition-all shadow-sm"
                        >
                                        ...
                                        <td className="px-8 py-5">
                                            {c.status === 'pending' ? (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <button
                                                        onClick={() => confirmDelete(c.id)}
                                                        className="size-9 rounded-xl bg-rose-500/10 text-rose-500 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all shadow-sm ml-auto"
                                                        title={t('dashboard.delete')}
                                                    >
                                                        <span className="material-symbols-outlined text-lg">delete</span>
                                                    </button>
                    ...
                    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl p-8 space-y-6">
                            <div className="space-y-2">
                                ...
                                <p className="text-sm text-slate-500">{t('dashboard.confirm_delete')}</p>
                            </div>
                            <div className="flex items-center gap-3">
